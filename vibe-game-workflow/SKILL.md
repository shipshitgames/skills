...
- **Game** = thin orchestrator. Builds a `GameContext` + a `GameSystems` registry, runs
  the rAF loop, exposes the public API by delegating to systems. See `fpsdemo/src/game/Game.ts`.
- **GameContext** = the shared mutable world (renderer, scene, camera, controls, clock,
  ...
  with `(container: HTMLElement, listener: StateListener)`. See `fpsdemo/src/game/context.ts`.
- **GameSystems** = a registry interface. Each system is `new XSystem(ctx, sys)` and calls
  ...
  in the registry to avoid runtime cycles. See `fpsdemo/src/game/systems.ts`.
...
One repo per game (e.g. `shipshitgames/<game>`). Match `fpsdemo/package.json`: Vite +
...
The `Game.start()` body literally is the build order — see `fpsdemo/src/game/Game.ts:54`:
...
6. **HUD** — `HudSystem.emit()` pushes `HUDState`; `HUD.tsx` renders health/ammo/score/banners.
   → load **shipshit-engine** for the StateListener pattern; see `fpsdemo/src/game/types.ts`.
...
Two pieces (see `fpsdemo/DEPLOY.md`):
...
- Reference canonical files by path (`fpsdemo/src/game/Game.ts`) instead of pasting them;
  re-read on demand.
...
- Each game is its own repo under `shipshitgames/`. Shared cross-game code (the
  ...
  package `@shipshit/engine` (still forming — `fpsdemo` is its proof). Until it ships,
  copy the patterns from `fpsdemo`; when it lands, depend on it and delete the duplicates.
...
3. Scaffold matching `fpsdemo/package.json`; empty systems registered in `systems.ts`.
