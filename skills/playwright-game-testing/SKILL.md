...
Grounded in the canonical fpsdemo — **read these before writing selectors**:

- `vite.config.ts` → dev server is **port 5173**, `host: true`.
- `index.html` → `<title>FPS Arena</title>`, app mounts in `#root`.
...
await page.goto('/')
await page.getByText('FPS ARENA').waitFor()        // the <h1>
...
test('survivors: firing into the swarm scores a kill', async ({ page }) => {
  await page.goto('/')
  await page.getByText('FPS ARENA').waitFor()
